...
func (l *ParityLogger) CaptureStart(env *EVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
...
func (l *ParityLogger) CaptureFault(pc uint64, op OpCode, gas, cost uint64, scope *ScopeContext, depth int, err error) {
}
...
func NewTxLogger(signer types.Signer, IsApricotPhase3 bool, baseFee *big.Int, blockHash common.Hash, blockNumber uint64, perFolder, perFile uint64) (*TxLogger, error) {
	...
	return &TxLogger{
		...
		encoder:         json.NewEncoder(sb),
		signer:          signer,
		isApricotPhase3: IsApricotPhase3,
		baseFee:         baseFee,
	}, nil
}

func (t *TxLogger) TransactionDumpLogger(index int, tx *types.Transaction, receipt *types.Receipt) error {
	// End print the log from the transaction dump

	from, _ := types.Sender(t.signer, tx)
	// Assign the effective gas price paid for avalanche use isApricotPhase3 field
	effectiveGasPrice := hexutil.Uint64(tx.GasPrice().Uint64())
	if t.isApricotPhase3 {
